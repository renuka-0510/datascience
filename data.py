...
def predictive_analysis(df, target_col=None):
    print("\n====== PREDICTIVE ANALYSIS ======\n")

    if target_col is None:
        if "Do you consider this area to be safe?" in df.columns:
            target_col = "Do you consider this area to be safe?"
    ...
    if target_col not in df.columns:
        ...
        print("Skipping predictive analysis.")
        return
...
def prescriptive_analysis(feature_importances):
    ...
    msg = (
        ...
        f"- For example, if 'Number of wine shops' or 'surveillance' features are high-impact, interventions there may yield better safety outcomes.\n"
    )
    print(msg)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python area_safety.py dataset.csv [optional_target_column]")
        return

    fname = sys.argv[1]
    target_col = sys.argv[2] if len(sys.argv) == 3 else None
    ...
    feature_importances = predictive_analysis(df_processed, target_col)
